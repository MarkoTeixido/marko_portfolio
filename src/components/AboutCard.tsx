    ...
    <section className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4 mb-4">
        <div className="bg-zinc-200 dark:bg-slate-700 flex max-sm:p-6 p-10 rounded-3xl">
          <div className="relative w-full">
            <div className="absolute top-0 -right-2 w-64 h-64 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000"></div>
              <div className="absolute top-0 -bottom-6 -right-2 w-64 h-64 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob"></div>
              <div className="absolute top-4 -bottom-8 -right-4 w-64 h-64 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-2000"></div>
              <div className="absolute -bottom-14 -right-6 max-sm:w-40 max-sm:h-40 sm:w-72 sm:h-72 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000"></div>
              <div className="absolute -bottom-16 -right-6 w-64 h-64 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000"></div>
              <div className="absolute -right-4 max-sm:w-36 max-sm:h-36 sm:w-72 sm:h-72 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000"></div>
              <div className="max-sm:m-2 md:m-8 relative space-y-4 flex flex-col gap-5">
                ...
                <div className="flex flex-col lg:flex-row items-center gap-4 md:pt-10 justify-self-end">
                  <a href={`/CV/CV_${context.language}.pdf`} download={`CV_${context.language}.pdf`} target='_blank' rel='noreferrer' className="dark:text-black dark:bg-slate-100 text-center bg-black text-white max-sm:font-normal font-medium py-3 px-12 max-sm:w-[14rem] rounded-full w-60 lg:w-auto hover:opacity-80 transition duration-200 hover:ease-linear">
